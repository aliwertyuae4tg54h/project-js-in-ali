...
function showProduct(data){
    allmeals.innerHTML=data.map((item) => {
        return `
                    <div onclick="goToMealId('${item.idMeal}')" class="category1"> 
...
function goToMealId(id){
  window.location.href= `vneshny.html?id=${id}`
}
...
let firstLetteer = document.querySelector(".letter");

firstLetteer.innerHTML  =arrayLetters.map((item) => {
  return`
